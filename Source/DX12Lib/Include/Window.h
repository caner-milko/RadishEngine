...
public:
	struct BackBuffer
	{
		...
		d3d12::CommandList* LastCmdList = nullptr;
	};
...
private:
	...
	std::weak_ptr<Game> GamePtr;

	struct Ticker
	{
		Clock::time_point Start, LastTime;
		Ticker() : Start(Clock::now()), LastTime(Start) {}
		float Update()
		{
			auto now = Clock::now();
			float diff = std::chrono::duration<float>(now - LastTime).count();
			LastTime = now;
			return diff;
		}
