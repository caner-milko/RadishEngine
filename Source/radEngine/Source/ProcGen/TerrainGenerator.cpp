...
namespace rad::proc
{
	...
	{
		HeightMapToTerrainMaterialPSO = PipelineState::CreateBindlessComputePipeline("HeightToTerrainMaterialPipeline", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/HeightMapToTerrainMaterial.hlsl");
		HeightMapToWaterMaterialPSO = PipelineState::CreateBindlessComputePipeline("HeightToWaterMaterialPipeline", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/HeightMapToWaterMaterial.hlsl");

		ThermalOutfluxPSO = PipelineState::CreateBindlessComputePipeline("ThermalErosionOutflux", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/T1ThermalOutflux.hlsl");
		ThermalDepositPSO = PipelineState::CreateBindlessComputePipeline("ThermalErosionDeposit", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/T2ThermalDeposit.hlsl");

		HydrolicAddWaterPSO = PipelineState::CreateBindlessComputePipeline("HydrolicAddWater", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/H1AddWater.hlsl");
		HydrolicCalculateOutfluxPSO = PipelineState::CreateBindlessComputePipeline("HydrolicCalculateOutflux", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/H2CalculateOutflux.hlsl");
		HydrolicUpdateWaterVelocityPSO = PipelineState::CreateBindlessComputePipeline("HydrolicUpdateWaterVelocity", Renderer, RAD_SHADERS_DIR L"Compute/Terrain/H3UpdateWaterVelocity.hlsl");
...

		{
			...
			TerrainDeferredPSO = PipelineState::Create("TerrainRender", Renderer.GetDevice(), deferredPSStream, &Renderer.ShaderManager->BindlessRootSignature);
			...
			TerrainDepthOnlyPSO = PipelineState::Create("TerrainDepthOnly", Renderer.GetDevice(), depthOnlyPSStream, &Renderer.ShaderManager->BindlessRootSignature);
		}

		{
			...
			auto [vertexShader, pixelShader] = Renderer.ShaderManager->CompileBindlessGraphicsShader(L"RenderWater", RAD_SHADERS_DIR L"Compute/Terrain/RenderWater.hlsl");
			...
			WaterForwardPSO = PipelineState::Create("WaterRender", Renderer.GetDevice(), waterPSStream, &Renderer.ShaderManager->BindlessRootSignature);
		}
	...
	CTerrain TerrainErosionSystem::CreateTerrain(uint32_t heightMapWidth)
	{
		...
		terrain.HeightMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"HeightMap", baseTextureInfo));
		terrain.WaterHeightMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"WaterHeightMap", baseTextureInfo));
		terrain.SedimentMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"SedimentMap", baseTextureInfo));
		terrain.TempHeightMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"TempHeightMap", baseTextureInfo));
		terrain.TempSedimentMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"TempSedimentMap", baseTextureInfo));
		terrain.SoftnessMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"HardnessMap", baseTextureInfo));

		baseTextureInfo.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
		terrain.WaterOutflux = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"WaterOutflux", baseTextureInfo));
		terrain.ThermalPipe1 = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"ThermalPipe1", baseTextureInfo));
		terrain.ThermalPipe2 = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"ThermalPipe2", baseTextureInfo));

		baseTextureInfo.Format = DXGI_FORMAT_R32G32_FLOAT;
		terrain.VelocityMap = std::make_shared<RWTexture>(DXTexture::Create(Renderer.GetDevice(), L"VelocityMap", baseTextureInfo));
		return terrain;
	}

	void TerrainErosionSystem::GenerateBaseHeightMap(CommandRecord& cmdRecord, CTerrain& terrain, CErosionParameters const& parameters, OptionalRef<CTerrainRenderable> terrainRenderable, OptionalRef<CWaterRenderable> waterRenderable)
	{
		//create heightmap
	...
	void TerrainErosionSystem::ErodeTerrain(CommandRecord& cmdRecord, CTerrain& terrain, CErosionParameters const& parameters, OptionalRef<CTerrainRenderable> terrainRenderable, OptionalRef<CWaterRenderable> waterRenderable)
	{
		for (int i = 0; i < parameters.Iterations; i++)
		{
			...
			cmdRecord.Push("Erosion", 
				[heightMap = terrain.HeightMap,
				 waterHeightMap = terrain.WaterHeightMap,
				 sedimentMap = terrain.SedimentMap,
				 tempHeightMap = terrain.TempHeightMap,
				 tempSedimentMap = terrain.TempSedimentMap,
				 softnessMap = terrain.SoftnessMap,
				 thermalPipe1 = terrain.ThermalPipe1,
				 thermalPipe2 = terrain.ThermalPipe2,
				 waterOutflux = terrain.WaterOutflux,
	...
	CIndexedPlane TerrainErosionSystem::CreatePlane(CommandRecord& cmdRecord, uint32_t resX, uint32_t resY)
	{
		...
		plane.Indices = std::make_shared<DXTypedBuffer<uint32_t>>(DXTypedBuffer<uint32_t>::Create(Renderer.GetDevice(), L"PlaneIdxBuffer", indices.size(), D3D12_HEAP_TYPE_DEFAULT));
	...
	CTerrainRenderable TerrainErosionSystem::CreateTerrainRenderable(CTerrain& terrain)
	{
		...
		renderable.TerrainAlbedoTex = std::make_shared<RWTexture>(RWTexture(DXTexture::Create(Renderer.GetDevice(), L"TerrainAlbedo", texInfo), -1));
		renderable.TerrainNormalMap = std::make_shared<RWTexture>(RWTexture(DXTexture::Create(Renderer.GetDevice(), L"TerrainNormal", texInfo), -1));
		return renderable;
	}
	...
	CWaterRenderable TerrainErosionSystem::CreateWaterRenderable(CTerrain& terrain)
	{
		...
		renderable.WaterAlbedoMap = std::make_shared<RWTexture>(RWTexture(DXTexture::Create(Renderer.GetDevice(), L"WaterAlbedo", texInfo), -1));
		renderable.WaterNormalMap = std::make_shared<RWTexture>(RWTexture(DXTexture::Create(Renderer.GetDevice(), L"WaterNormal", texInfo), -1));
		return renderable;
	}

	void TerrainErosionSystem::GenerateTerrainMaterial(CommandRecord& cmdRecord, CTerrain& terrain, CErosionParameters const& parameters, CTerrainRenderable& renderable)
	{
		cmdRecord.Push("GenerateTerrainMaterial",
			...
			dispatchSize = std::pair<uint32_t, uint32_t>(renderable.TerrainNormalMap->Info.Width / 8, renderable.TerrainNormalMap->Info.Height / 8),
	...
	void TerrainErosionSystem::GenerateWaterMaterial(CommandRecord& cmdRecord, CTerrain& terrain, CErosionParameters const& parameters, CWaterRenderable& renderable)
	{
		cmdRecord.Push("GenerateWaterMaterial",
			...
			sedimentMap = terrain.SedimentMap,
			cellSize = parameters.TotalLength / terrain.HeightMap->Info.Width,
			dispatchSize = std::pair<uint32_t, uint32_t>(renderable.WaterNormalMap->Info.Width / 8, renderable.WaterNormalMap->Info.Height / 8),
	...
	void TerrainErosionSystem::Update(entt::registry& registry, InputManager& inputMan, RenderFrameRecord& frameRecord)
	{
		...
		frameRecord.Push(TypedRenderCommand<WaterRenderData>{.Name = "WaterRender", .Data = std::move(waterRenderDataVec),
			.ForwardPass = [this](auto span, auto view, auto passData) {WaterForwardPass(span, view, passData); }
		});
	}

	void TerrainErosionSystem::TerrainDepthOnlyPass(std::span<TerrainRenderData> renderObjects, const RenderView& view, DepthOnlyPassData& passData)
	{
	...
	void TerrainErosionSystem::WaterForwardPass(std::span<WaterRenderData> renderObjects, const RenderView& view, ForwardPassData& passData)
	{
		auto& cmd = passData.CmdContext;
		cmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
