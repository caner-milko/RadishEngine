...
bool ShaderManager::Init()
{
	...
	auto vertexShader = CompileBindlessVertexShader(L"Fullscreen", RAD_ENGINE_SHADERS_DIR L"FullscreenVS.hlsli");
	ThrowIfFailed(Renderer.GetDevice().CreateRootSignature(0u,
														   vertexShader->RootSignatureBlob->GetBufferPointer(),
...
{
	std::vector<std::wstring_view> includeFoldersCopy{{RAD_ENGINE_SHADERS_DIR L""}};
...
Shader* ShaderManager::CompileBindlessVertexShader(std::wstring_view name,
...
Shader* ShaderManager::CompileBindlessComputeShader(std::wstring_view name,
													std::wstring_view shaderPath,
													std::wstring_view entryPoint,
													std::span<const std::wstring_view> includeFolders)
{
...
Shader* ShaderManager::CompileShader(std::wstring_view name,
									 std::wstring_view shaderPath,
									 ShaderType type,
									 std::wstring_view entryPoint,
									 std::span<const std::wstring_view> includeFolders)
{
	...
	switch (type)
	{
	case ShaderType::Vertex: shaderType = L"vs_6_6"; break;
	case ShaderType::Pixel: shaderType = L"ps_6_6"; break;
	case ShaderType::Compute: shaderType = L"cs_6_6"; break;
	}
	...
	Compiler->Compile(
		&Source, compilationArgs.data(), compilationArgs.size(), IncludeHandler.Get(), IID_PPV_ARGS(&results));
