...
namespace rad
{
DXTexture DXTexture::Create(RadDevice& device, std::wstring name, TextureCreateInfo const& info,
							D3D12_RESOURCE_STATES startState)
{
	// Create the texture
	auto desc = CD3DX12_RESOURCE_DESC::Tex2D(
		info.Format, info.Width, info.Height, info.DepthOrArraySize * (info.IsCubeMap ? 6 : 1), info.MipLevels,
		info.SampleDesc.Count, info.SampleDesc.Quality, info.Flags, info.Layout, info.Alignment);
	auto heapProp = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);

	ComPtr<ID3D12Resource> resource;

	device.CreateCommittedResource(&heapProp, D3D12_HEAP_FLAG_NONE, &desc, startState,
								   info.ClearValue ? &*info.ClearValue : nullptr, IID_PPV_ARGS(&resource));

	return DXTexture(name, resource, startState, info);
}

DXTexture DXTexture::FromExisting(RadDevice& device, std::wstring name, ComPtr<ID3D12Resource> resource,
								  TextureCreateInfo const& info, D3D12_RESOURCE_STATES startState)
{
	return DXTexture(name, resource, startState, info);
}
...
void DXTexture::UploadData(CommandContext& commandCtx, std::span<const std::byte> data, uint8_t bytesPerPixel)
{
	...
	auto intermediateBuf =
		DXBuffer::Create(commandCtx.Device, Name + L"_IntermediateBuffer", data.size(), D3D12_HEAP_TYPE_UPLOAD);
	...
	uint64_t res = UpdateSubresources(&commandCtx.CommandList, Resource.Get(), intermediateBuf.Resource.Get(), 0, 0, 1,
									  &subresourceData);
	assert(res != 0);
}
...
DXBuffer DXBuffer::Create(RadDevice& device, std::wstring name, size_t size, D3D12_HEAP_TYPE heapType,
						  D3D12_RESOURCE_FLAGS flags)
{
	...
	device.CreateCommittedResource(&heapProp, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON, nullptr,
								   IID_PPV_ARGS(&resource));
	return DXBuffer(name, resource, size);
}

DXBuffer DXBuffer::CreateAndUpload(RadDevice& device, std::wstring name, CommandContext& commandCtx,
								   std::span<const std::byte> data, D3D12_RESOURCE_STATES state,
								   D3D12_RESOURCE_FLAGS flags)
{
...
UnorderedAccessView DXBuffer::CreateUAV(size_t numElements, size_t stride, size_t firstElement,
...
void DXBuffer::CreatePlacedSRV(DescriptorAllocationView alloc, size_t numElements, size_t stride, size_t offset,
...
void DXBuffer::CreatePlacedUAV(DescriptorAllocationView alloc, size_t numElements, size_t stride, size_t firstElement,
							   D3D12_BUFFER_UAV_FLAGS flags)
{
	if (numElements == 0)
		numElements = stride == 0 ? Size : Size / stride;
