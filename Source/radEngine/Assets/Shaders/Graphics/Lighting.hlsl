...
float shadow_offset_lookup(
	Texture2D<float> shadowMap, SamplerComparisonState shadowMapSampler, float3 loc, float2 offset, float bias)
{
...
float4 PSMain(PSIn IN) : SV_TARGET
{
	...
	float halfVector = saturate(
		dot(normal,
			normalize(-lightData.DirectionOrPosition + -mul((float3x3)viewTransform.CamInverseView, float3(0, 0, 1)))));
	...
	bool inBounds = lightSpacePos.x > 0 && lightSpacePos.x < 1 && lightSpacePos.y > 0 && lightSpacePos.y < 1 &&
	...
	if (inBounds)
	{
		float sum = 0;
		float x, y;
		static const float2 poissonDisk[16] = {float2(-0.94201624, -0.39906216),
	...
	// return lerp(float4((diffuse * lightData.Color + float3(0.1, 0.1, 0.1) * specular + lightData.AmbientColor) *
	// albedo, 1), float4(albedoTex.Sample(PointSampler, reflectionUv.xy).rgb, 1), reflectionUv.a);
